...
public class Return extends ExecutableCard {

	public Return(Player user, Card card) {
		super(user, card, CardFunction.RETURN);
	}

	@Override
	public int execute(Game game) {
		game.operate().triggerReturn(card, user);
		return Stack.ONE_MORE_USAGE;
	}

	@Override
	public boolean invalidate(ExecutableCard exe) {
		return exe instanceof Return;
	}
