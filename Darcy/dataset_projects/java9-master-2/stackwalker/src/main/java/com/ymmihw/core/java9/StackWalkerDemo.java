...
public class StackWalkerDemo {
  ...
  public void methodThree() {
    ...
    stackTrace =
        StackWalker.getInstance(StackWalker.Option.SHOW_REFLECT_FRAMES).walk(this::walkExample);
    ...
    Runnable r = () -> {
      List<StackFrame> stackTrace2 =
          StackWalker.getInstance(StackWalker.Option.SHOW_HIDDEN_FRAMES).walk(this::walkExample);
  ...
  public void printStackTrace(List<StackFrame> stackTrace) {
    for (StackFrame stackFrame : stackTrace) {
      System.out.println(stackFrame.getClassName().toString() + "#" + stackFrame.getMethodName()
