...
public class UsageController {
	...
	private class CustomListener implements SkillBoxListener, ButtonListener, HandBoxListener {
		...
		public void onHandToggled(HandView hand, boolean status) {
			...
			gameView.getInstruction().activate("確定使用這張 " + card.getFunction().getName()+ " ？", true, true);
		}
	...
	public UsageController(Map<Card, ? extends Action> handCandidates, Map<String, ? extends Action> skillCandidatesm, GameView gameView) {
	...
	public void run() {
		gameView.getInstruction().activate("出牌階段，請選擇要使用的卡牌或技能。", false, true);
